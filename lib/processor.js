 ...
 */
 ...
 Processor.prototype.castValue = function(table, key, value, attributes) {
  ...
  if (self.schema[identity]._columnsMap[key]) {
    attr = this.schema[identity]._columnsMap[key];
  }

  if(!attr) attr = key;

  // Lookup Schema "Type"
  if(!self.schema[identity] || !self.schema[identity].attributes[attr]) return;

  if(!_.isPlainObject(self.schema[identity].attributes[attr])) {
    type = self.schema[identity].attributes[attr];
  } else {
    type = self.schema[identity].attributes[attr].type;
  }

  switch (type) {
    case 'array':
      try { attributes[key] = JSON.parse(value); } catch (e) {}
      break;
    case 'float':
      if (null !== value && undefined !== value) {
        try { attributes[key] = parseFloat(value); } catch (e) {}
      }
      break;
  }
